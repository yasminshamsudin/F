...
# 2021-09-08 Added command for neutralizing charged systems
...
runDirectory=210913-AVBNagd                        # Path to the ligand directory
...
for ligname in *
 do
	...
	echo -e "module load chemistry gromacs" >> COMPLEX.job
        ...
        # Neutralize system
        echo -e "gmx grompp -f ../ions.mdp -c COMPLEX.gro -p COMPLEX.top -o COMPLEX_ions.tpr" >> COMPLEX.job
        echo -e "echo SOL | gmx genion -s COMPLEX_ions.tpr -o COMPLEX_ions.gro -p COMPLEX.top -pname NA -nname CL -neutral"'\n' >> COMPLEX.job

        # Run minimization
        echo -e "gmx grompp -f ../min.mdp -c COMPLEX_ions.gro -p COMPLEX.top -o COMPLEX_min.tpr" >> COMPLEX.job
