...
FF=g                            # Forcefield options: g (GAFF, default), o (OPLSAA), a (AMBER 14SB)
...
boxsizeSmallest=15              # default radius (in Å): 20
boxsizeLargest=20
boxsizeSteps=5
     ...
     do
        ...
        for i in *.pdb   # Repeat for all solvents in the SOLVENTS folder
          do
            ...
            gmx insert-molecules -f $ligname"_"$boxsize.gro -ci $workingDirectory/$solventDirectory/$solvname.pdb -nmol $nsolvmol -o ${ligname}_${solvname}_${boxsize}.gro
            wait
                
	        # Create master topology (.top) files 

                # Get the header until atom types from ligand file
                sed -n -r '/LIG_GMX/,/Amb/p' $ligname"_GMX.top" > header.tmp    
