...
probeAtomno1=10                               # Default for automated C atomnumber allocation: 0
probeAtomno2=12                               # Default for automated atomnumber allocation: 0

ligandDirectory=PREP                     # Path to the ligand directory
ion=ION					# NA for Na+, ION for tBA

############ Advanced modelling. Only edit if you know what you are doing! #################

workingDirectory=$(pwd)                     # Path to where this script is located

tempK=298				# Temperature in K (298K = 25C)
...
# Create the probe atoms list file (.ndx) and the python script for calculating electric fields
             ...
             if [[ $probeAtom1=C && $probeAtom2=O ]]
                then
                   ...
                   if [[ $probeAtomno1 = 0 && $probeAtomno2 = 0 ]]
                        ...
                        # Get the probe atom charges
                        grep "\s$carbonno\s" ligatomtypes.tmp > cprobe.tmp
        ...
        # Make the Electric field calculation script (BLCO.py)
...
# Create neutralizing, minimization, heating, equilibration, and md run input files (.mdp)
        ...
        echo -e "tc-grps		= LIG    $ion   SOL	; two coupling groups - more accurate" >> nvt.mdp
        echo -e "tau_t		= 2	  2	  2           ; time constant, in ps" >> nvt.mdp
        echo -e "ref_t		= $tempK 	  $tempK    $tempK       ; reference temperature, one for each group, in K"'\n' >> nvt.mdp
        ...
        echo -e "gen_temp	= $tempK		; temperature for Maxwell distribution" >> nvt.mdp
        ...
        echo -e "tc-grps		= LIG    $ion    SOL	; two coupling groups - more accurate" >> npt.mdp
        echo -e "tau_t		= 2	  2       2    ; time constant, in ps" >> npt.mdp
        echo -e "ref_t		= $tempK 	  $tempK     $tempK      ; reference temperature, one for each group, in K"'\n' >> npt.mdp
        ...
        echo -e "tc-grps		= LIG    $ion    SOL	; two coupling groups - more accurate" >> md.mdp
        echo -e "tau_t		= 2	  2       2    ; time constant, in ps" >> md.mdp
        echo -e "ref_t		= $tempK 	  $tempK     $tempK      ; reference temperature, one for each group, in K"'\n' >> md.mdp
